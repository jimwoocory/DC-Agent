...
DEFAULT_KB_INBOX_DOWNLOAD_DIR = (
    PROJECT_ROOT / "nas" / "knowledge" / "inbox" / "download"
)
SUPPORTED_LOCAL_IMPORT_SUFFIXES = {
    ".docx",
    ".epub",
    ".md",
    ".markdown",
...
class KnowledgeBaseRoute(Route):
    ...
    def __init__(
        ...
        self.routes = {
            ...
            "/kb/document/import/local-dir": (
                "POST",
                self.import_documents_from_local_dir,
            ),
    ...
    async def import_documents(self):
        ...
        - documents: 文档列表 (必填)
            ...
            - source_path: 原始来源路径 (可选, 用于后续来源引用)
    ...
    def _resolve_local_import_dir(self, raw_dir: str | None) -> Path:
        base_dir = DEFAULT_KB_INBOX_DOWNLOAD_DIR.resolve()
        candidate = Path(raw_dir).expanduser() if raw_dir else base_dir
        if not candidate.is_absolute():
            candidate = PROJECT_ROOT / candidate
        resolved = candidate.resolve()
        if resolved != base_dir and base_dir not in resolved.parents:
            raise ValueError(f"只允许导入 {base_dir} 下的文件")
    ...
    def _collect_local_import_files(
        ...
        *,
        ...
        for path in sorted(iterator):
            if not path.is_file():
                continue
            if path.name.startswith(".") or path.suffix == ".download":
                continue
            suffix = path.suffix.lower()
            if suffix not in SUPPORTED_LOCAL_IMPORT_SUFFIXES:
                continue
            ...
            if max_bytes > 0 and size > max_bytes:
                files.append(
                    {
                        "file_name": path.name,
                        "error": f"文件超过大小限制 {max_file_mb}MB",
            ...
            if len([item for item in files if "file_content" in item]) >= max_files:
                break
        return files

    async def import_documents_from_local_dir(self):
        """从本地 NAS inbox/download 目录导入知识库文档。

        Body:
        - kb_id: 知识库 ID (必填)
        - directory: 导入目录 (可选，默认 nas/knowledge/inbox/download)
        - recursive: 是否递归扫描 (可选，默认 false)
        - max_files: 单次最多导入文件数 (可选，默认 20)
        - max_file_mb: 单文件大小限制 MB (可选，默认 80)
        """
        try:
            kb_manager = self._get_kb_manager()
            data = await request.json
            ...
            directory = self._resolve_local_import_dir(data.get("directory"))
            recursive = bool(data.get("recursive", False))
            max_files = max(1, min(int(data.get("max_files", 20)), 100))
            max_file_mb = max(1, min(int(data.get("max_file_mb", 80)), 500))

            collected = self._collect_local_import_files(
                directory,
                recursive=recursive,
                max_files=max_files,
                max_file_mb=max_file_mb,
            )
            skipped = [item for item in collected if "error" in item]
            files_to_upload = [item for item in collected if "file_content" in item]
            if not files_to_upload:
                return (
                    Response()
                    .error(
                        "目录内没有可导入文件。当前支持: "
                        + ", ".join(sorted(SUPPORTED_LOCAL_IMPORT_SUFFIXES))
                    )
                    .__dict__
                )
            ...
            asyncio.create_task(
                self._background_upload_task(
                    ...
                    tasks_limit=int(data.get("tasks_limit", 3)),
                ...
                .ok(
                    {
                        ...
                        "file_count": len(files_to_upload),
                        "skipped": skipped,
                        "message": "task created, processing local inbox files in background",
                    },
                )
                .__dict__
            )
            ...
            logger.error(f"从本地目录导入文档失败: {e}")
            logger.error(traceback.format_exc())
            return Response().error(f"从本地目录导入文档失败: {e!s}").__dict__
