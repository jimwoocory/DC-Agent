...
任何路由决策应该先看 ``antigravity_health`` / ``qwen_health`` 的 circuit
...
def health_snapshot() -> dict[str, Any]:
    ...
    agy = get_antigravity_health()
    qwen = get_qwen_health()
    return {
        "antigravity": {
            "provider_id": agy.get("provider_id", ""),
            "available": bool(agy.get("available", True)),
            "status": agy.get("status", ""),
            "reason": agy.get("reason", ""),
            "remaining_seconds": int(agy.get("remaining_seconds") or 0),
            "consecutive_failures": int(agy.get("consecutive_failures") or 0),
            "last_error_code": agy.get("last_error_code", ""),
        },
        "qwen3.6_flash": {
