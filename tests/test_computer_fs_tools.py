...
def _make_context(
    *,
    ...
    if harness_store is not None:
        config_holder.harness_store = harness_store
...
@pytest.mark.asyncio
async def test_restricted_member_can_read_active_truth_intake_archive(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
):
    _setup_local_fs_tools(monkeypatch, tmp_path)
    archive_dir = (
        tmp_path / "data" / "harness_intake" / "raw" / "20260603" / "intake123"
    )
    archive_dir.mkdir(parents=True)
    source_file = archive_dir / "message.txt"
    source_file.write_text("公众号链接：https://example.com/source", encoding="utf-8")
    ...
    result = await fs_tools.FileReadTool().call(
        _make_context(role="member", harness_store=store),
        path=str(source_file),
    )

    assert "公众号链接" in result


@pytest.mark.asyncio
async def test_restricted_member_grep_defaults_include_active_truth_intake_archive(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
):
    _setup_local_fs_tools(monkeypatch, tmp_path)
    ...
    archive_dir.mkdir(parents=True)
    (archive_dir / "message.txt").write_text(
        "真实素材链接：https://example.com/material",
        encoding="utf-8",
    )
    store = _FakeHarnessStore(
        [
    ...
    assert "example.com/material" in result


@pytest.mark.asyncio
async def test_restricted_member_broad_data_grep_narrows_to_active_truth_intake(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
):
    _setup_local_fs_tools(monkeypatch, tmp_path)
    data_root = tmp_path / "data"
    archive_dir = data_root / "harness_intake" / "raw" / "20260603" / "intake123"
    archive_dir.mkdir(parents=True)
    (archive_dir / "message.txt").write_text(
        "本次真实链接：https://example.com/current",
        encoding="utf-8",
    )
    temp_dir = data_root / "temp"
    temp_dir.mkdir()
    (temp_dir / "old-index.md").write_text(
        "旧历史链接：https://example.com/stale",
        encoding="utf-8",
    )
    store = _FakeHarnessStore(
        [
    ...
    assert "example.com/current" in result
    assert "example.com/stale" not in result


@pytest.mark.asyncio
async def test_restricted_member_still_cannot_read_unrelated_data_path(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
):
    _setup_local_fs_tools(monkeypatch, tmp_path)
    unrelated = tmp_path / "data" / "private.txt"
    unrelated.write_text("secret", encoding="utf-8")

    result = await fs_tools.FileReadTool().call(
        _make_context(role="member", harness_store=_FakeHarnessStore([])),
        path=str(unrelated),
    )
