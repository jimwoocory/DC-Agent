...
class KnowledgeBaseManager:
    ...
    def _format_context(self, results: list[RetrievalResult]) -> str:
        ...
        for i, result in enumerate(results, 1):
            ...
            if source_path:
                ...
                lines.append(f"来源引用: {self._obsidian_wikilink_for_result(result)}")
    ...
    @staticmethod
    def _obsidian_wikilink_for_result(result: RetrievalResult) -> str:
        source_path = result.metadata.get("source_path") or result.doc_name
        title = Path(str(source_path)).stem or Path(result.doc_name).stem
        return f"[[{title}]]"
