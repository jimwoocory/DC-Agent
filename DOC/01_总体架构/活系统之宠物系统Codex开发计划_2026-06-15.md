...
- `Pet Live Core` 已新增到 `dc_engines/dc_engines/pet_live/`，包含 identity、event bus、reducer、SQLite store、service、integration helpers、Obsidian bridge。
- 飞书宠物助手、DC Router、Harness Engine、Hermes Bridge、Obsidian/KB 记忆治理已接入同一套 `pet_id`、事件 outbox 和状态快照。
- AstrBot Dashboard 已新增 `/api/pet/*`，桌面端可读取当前状态、增量事件、心跳、动作、资源包。
- 已新增 `/api/pet/bind-desktop` 服务端绑定口：由可信 OAuth/飞书会话层携带 `PET_LIVE_BINDING_TOKEN`，把 `dc_feishu_session` 绑定到飞书 `open_id`，桌面端不直接声明身份。
- DC-Agent 和 dianchi-desktop 已放入同一套 `orange-cat-v1` 宠物资源包，包含中文 manifest 和 `spritesheet.webp`。
- 全局灰度总阀 `PET_LIVE_ENABLED` 已覆盖飞书、Router、桌面 API、Harness、Hermes；局部开关保留 `PET_LIVE_API_ENABLED`、`PET_LIVE_TAP_ENABLED`、`DIANCHI_PET_SCENE_ENABLED`。

最新验证：

- 根目录宠物闭环套件：`155 passed, 3 warnings`
- `dc_engines` Harness/Obsidian 套件：`73 passed, 2 warnings`

下一步优先级：

1. 桌面端从服务端资源 API 拉取并缓存宠物资源，当前已具备本地同包加载能力。
2. 飞书 `/pet` 卡片展示 live state，并给出桌面端打开/绑定入口。
3. 用真实 OAuth/公司主页服务调用 `/api/pet/bind-desktop`，完成生产身份绑定链路。
4. 替换 `orange-cat-v1` 的程序化 spritesheet 为更接近官方宠物主页的设计资产。
