...
class TestApplyKb:
    ...
    async def test_apply_kb_without_agentic_mode(self, mock_event, mock_context):
        ...
        assert "[Related Knowledge Base Results]:\nKB result" in kb_part.text
        assert "引用" in kb_part.text
        assert "来源路径" in kb_part.text

        message = Message.model_validate(await req.assemble_context())
        assert isinstance(message.content, list)
        assert message.content[0].text == "test question"
        assert message.content[1].text == kb_part.text
    ...
    def test_knowledge_base_tool_description_requires_source_citations(self):
        module = ama
        tool = module.KnowledgeBaseQueryTool()

        assert "source_path" in tool.description
        assert "cite" in tool.description.lower()
