...
class TestDiscoverBayCredentials:
    ...
    def test_discover_from_bay_data_dir_env(
        ...
        self._write_creds(cred_file, api_key="${REDACTED_API_KEY}")
    ...
    def test_endpoint_mismatch_still_returns_key(
        ...
        cred_file = data_dir / "credentials.json"
        self._write_creds(
            cred_file, api_key="${REDACTED_API_KEY}", endpoint="http://other-host:9000"
        )
        monkeypatch.setenv("BAY_DATA_DIR", str(data_dir))
    ...
    def test_bay_data_dir_priority_over_cwd(
        ...
        self._write_creds(env_dir / "credentials.json", api_key="${REDACTED_API_KEY}")
        self._write_creds(cwd_dir / "credentials.json", api_key="${REDACTED_API_KEY}")
...
class TestLogComputerConfigChanges:
    ...
    def test_masks_token_values(self, mock_logger) -> None:
        ...
        new = {
            "provider_settings": {
                "sandbox": {"shipyard_neo_access_token": "${REDACTED_API_KEY}"}
            }
        }
        ...
        assert "${REDACTED_API_KEY}" not in call_args_str
