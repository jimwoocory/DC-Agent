...
class _ContextProxy:
    def __init__(self, var, *, quart_attr: str | None = None) -> None:
        self._var = var
        self._quart_attr = quart_attr

    def _resolve(self):
        try:
            return self._var.get()
        except LookupError:
            if not self._quart_attr:
                raise
            try:
                import quart

                return getattr(quart, self._quart_attr)
    ...
    def __setattr__(self, key: str, value: Any) -> None:
        if key in {"_var", "_quart_attr"}:
            super().__setattr__(key, value)
            return
        setattr(self._resolve(), key, value)


request = _ContextProxy(_request_var, quart_attr="request")
websocket = _ContextProxy(_websocket_var)
g = _ContextProxy(_g_var, quart_attr="g")
current_app = _ContextProxy(_app_var, quart_attr="current_app")
    ...
    try:
        ...
        try:
            ...
            if has_request_context():
                return await quart_send_file(path, mimetype=mimetype, **kwargs)
        except ImportError:
            pass
