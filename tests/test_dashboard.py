...
def _build_memory_governance_test_app(dc_root: Path) -> FastAPIAppAdapter:
    test_app = FastAPIAppAdapter(FastAPI())
    MemoryGovernanceRoute(RouteContext(config=None, app=test_app), dc_root=dc_root)
    return test_app


@pytest.mark.asyncio
async def test_memory_governance_status_does_not_create_store(tmp_path: Path):
    dc_root = tmp_path / "dc-agent"
    (dc_root / "data").mkdir(parents=True)
    app = _build_memory_governance_test_app(dc_root)

    response = await app.test_client().get("/api/memory-governance/status")
    data = await response.get_json()

    assert response.status_code == 200
    assert data["status"] == "ok"
    assert data["data"]["total"] == 0
    assert data["data"]["store_exists"] is False
    assert not (dc_root / "data" / "governed_memory.db").exists()


@pytest.mark.asyncio
async def test_memory_governance_export_creates_obsidian_note(tmp_path: Path):
    dc_root = tmp_path / "dc-agent"
    data_dir = dc_root / "data"
    data_dir.mkdir(parents=True)
    (dc_root / "ObsidianVault").mkdir()
    nas_db = data_dir / "nas_memory.db"
    with sqlite3.connect(nas_db) as conn:
        ...
        conn.execute(
            ...
            (
                "doc-1",
                "projects/demo.md",
                "/nas/projects/demo.md",
                "abc123",
                "markdown",
                "Demo Project Memory",
                "The approved owner is Dianchi.",
                '["project"]',
    ...
    assert response.status_code == 200
    assert data["status"] == "ok"
    assert data["data"]["exported_count"] == 1
    assert (dc_root / "data" / "governed_memory.db").exists()
    note_paths = data["data"]["note_paths"]
    assert len(note_paths) == 1
    note = Path(note_paths[0])
    assert note.exists()
    assert "review_status: need_review" in note.read_text(encoding="utf-8")
...
@pytest.mark.asyncio
async def test_pet_live_routes_bypass_dashboard_jwt(app: FastAPIAppAdapter):
    test_client = app.test_client()
