...
_TRUTH_INTAKE_SOURCE = "llm_router_truth_intake"
_ACTIVE_INTAKE_STATUSES = ("pending", "in_progress", "blocked", "review_required")
    ...
    *,
    ...
    labels.extend(
        [
            ...
            get_astrbot_system_tmp_path(),
            get_astrbot_temp_path(),
            "active harness intake archives",
        ]
    )
    return labels
...
async def _active_intake_read_roots(
    context: ContextWrapper[AstrAgentContext],
    umo: str,
) -> tuple[Path, ...]:
    """Return read-only roots for source materials attached to active truth tasks."""
    store = getattr(context.context.context, "harness_store", None)
    if store is None or not umo:
        return ()

    data_intake_root = (Path(get_astrbot_data_path()) / "harness_intake").resolve(
        strict=False
    )
    try:
        tasks = await store.list_tasks_for_session(
            umo,
            limit=5,
            statuses=_ACTIVE_INTAKE_STATUSES,
        )
    except Exception as exc:  # noqa: BLE001
        logger.debug("[fs_tools] active intake roots lookup failed: %s", exc)
        return ()
    ...
    for task in tasks:
        ...
        if payload.get("source") != _TRUTH_INTAKE_SOURCE:
            continue
        raw_archive_dir = str(payload.get("archive_dir") or "").strip()
        if not raw_archive_dir:
            continue
        archive_dir = Path(raw_archive_dir).expanduser().resolve(strict=False)
        if not (
            archive_dir == data_intake_root
            or archive_dir.is_relative_to(data_intake_root)
        ):
            logger.warning(
                "[fs_tools] ignored truth intake archive outside data root: %s",
                archive_dir,
            )
            continue
        if archive_dir not in seen:
            roots.append(archive_dir)
            seen.add(archive_dir)
    return tuple(roots)


async def _read_allowed_roots_for_context(
    context: ContextWrapper[AstrAgentContext],
    umo: str,
    current_workspace_root: Path | None = None,
) -> tuple[Path, ...]:
    active_roots = await _active_intake_read_roots(context, umo)
    return active_roots + _read_allowed_roots(umo, current_workspace_root)
        ...
        try:
            normalized_path = (
                _normalize_read_path(
                    ...
                    local_env=local_env,
                    umo=umo,
                    allowed_roots=await _read_allowed_roots_for_context(
                        context,
                        umo,
                        current_workspace_root,
                    ),
    ...
    def _normalize_search_paths(
        ...
        *,
        ...
        if restricted:
            ...
            if disallowed:
                for blocked_path in disallowed:
                    requested = Path(blocked_path).resolve(strict=False)
                    narrowed.extend(
                        str(root)
                        for root in allowed_roots
                        if root == requested or root.is_relative_to(requested)
                    )
                if narrowed:
                    return narrowed
                allowed = ", ".join(
