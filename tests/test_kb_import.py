...
class _FakeKBDB:
    def __init__(self):
        self.session = _FakeKBSession()

    def get_db(self):
        return self.session

    async def update_kb_stats(self, **kwargs):
        return None
...
@pytest.mark.asyncio
async def test_import_documents_passes_source_path_to_kb_helper(
    core_lifecycle_td: AstrBotCoreLifecycle,
):
    kb_helper = await core_lifecycle_td.kb_manager.get_kb("test_kb_id")
    kb_helper.upload_document.reset_mock()
    ...
    await KnowledgeBaseRoute._background_import_task(
        ...
        documents=[
            {
                "file_name": "customer-a-delivery.md",
                "chunks": ["delivery note chunk"],
...
@pytest.mark.asyncio
async def test_upload_document_persists_source_path_as_file_path():
    ...
    helper.kb_db = _FakeKBDB()
    helper.vec_db = MagicMock()
    helper.vec_db.insert_batch = AsyncMock()
    helper._ensure_vec_db = AsyncMock(return_value=helper.vec_db)
    helper.refresh_kb = AsyncMock()
    helper.refresh_document = AsyncMock()

    doc = await KBHelper.upload_document(
        helper,
        file_name="customer-a-delivery.md",
        file_content=None,
        file_type="md",
        pre_chunked_text=["delivery note chunk"],
        source_path="/NAS/FeishuDocs/客户A/交付方案.md",
    )

    assert doc.file_path == "/NAS/FeishuDocs/客户A/交付方案.md"
    assert helper.kb_db.session.added[0].file_path == (
        "/NAS/FeishuDocs/客户A/交付方案.md"
    )
