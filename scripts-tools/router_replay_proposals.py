#!/usr/bin/env python3
"""Build review-only router replay proposals from observation JSONL."""
...
DEFAULT_OUTPUT_PATH = (
    ...
    / "replay"
    / "proposals"
    / "router_decision_framework_proposals.json"
)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create review-only router replay proposals from observations."
    )
    parser.add_argument(
        "--observations",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Proposal JSON output path.",
    )
    parser.add_argument("--limit", type=int, default=50)
    args = parser.parse_args()

    output_path = write_replay_proposals(
